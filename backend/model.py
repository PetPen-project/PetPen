...
class backend_model():
    ...
    def load_dataset(self,model_path,dataset_path,validate_path=None,validate_rate=0.3,shuffle=False):
        ...
        if validate_path:
            validate_set = pd.read_csv(validate_path)
        else:
            validate_set = dataset.iloc[int(dataset.shape[0]*(1-validate_rate)):]
            dataset = dataset.iloc[:int(dataset.shape[0]*(1-validate_rate))]

        self.train_sample_size = dataset.shape[0]
        self.validate_sample_size = validate_set.shape[0]
        ...
        for _in in self.inputs:
            ...
            self.train_x.append(dataset.loc[:,features].values)
            self.valid_x.append(validate_set.loc[:,features].values)
        ...
        for _out in self.outputs:
            ...
            if self.loss == 'categorical_crossentropy':
                ...
                self.train_y.append(to_categorical(dataset.loc[:,targets].values))
                self.valid_y.append(to_categorical(validate_set.loc[:,targets].values))
            else:
